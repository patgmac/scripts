...
# Emory University McAfee ePO installer for 10.6 and later
...
pkgTar="MSM120-ePO-1444.pkg.tar.gz"
pkgName="MSM120-ePO-1444.pkg"
...
if [ $osvers -le 5 ]; then
	echo "This installer is for OS versions 10.6 and higher"
...
if [ -e /usr/local/McAfee/uninstallMSC ]; then
	echo "McAfee installed already, uninstalling..."
	echo >> "${LogLoc}"
	echo >> "${LogLoc}"
	echo >> "${LogLoc}"
...
if [ -e "${pkgName}" ]; then
	...
	sudo installer -pkg "${pkgName}" -target / >>"${LogLoc}" 2>&1 
		...
		if [[ ! -e "/Applications/McAfee Security.app" ]]; then
			echo "ERROR: McAfee VirusScan not properly installed" >> "${LogLoc}"
			echo "ERROR: McAfee VirusScan not properly installed"
			exit 1
		fi
	sleep 10
...
### Firewall hotfix. Remove after next version ###
# Stop the Firewall
sh /usr/local/McAfee/Firewall/bin/FirewallControl stop 2>&1

# Renameing the old file to FWservice_org
sudo mv /usr/local/McAfee/Firewall/bin/FWService /usr/local/McAfee/Firewall/bin/FWService_org

# cpoy the newbuild FWService file to the below locatiion 
sudo cp -r FWService /usr/local/McAfee/Firewall/bin/ 2>&1

# change the ownship and group of the copied file
sudo chown root:Virex /usr/local/McAfee/Firewall/bin/FWService

# change the persmsion of the file
sudo chmod 775 /usr/local/McAfee/Firewall/bin/FWService

# start The firewall
sh /usr/local/McAfee/Firewall/bin/FirewallControl start 2>&1
### End firewall hotfix
